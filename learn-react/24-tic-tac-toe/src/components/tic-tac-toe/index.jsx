...
export default function TicTacToe() {
  ...
  // - 게임 위너!! [ ]
  // - 무승부! 게임 위너가 없습니다.
  let statusMessage = `플레이어: ${nextPlayer}`
  if (winner) statusMessage = `승자!: ${winner.player}`
  if (isDraw) statusMessage = '무승부!'
...
function SquaresGrid({ winner, squares, onPlay }) {
  const handleKeyControls = (e) => {
    ...
    // 위 나열된 키 외에는 브라우저 기본 작동 방지
    // ArrowLeft, ArrowRight, ArrowUp, ArrowDown, Escape
    e.preventDefault()

    // 이벤트 대상(target)으로부터
    // 현재 초점이 이동된 그리스 셀의 행/열 순서(인덱스) 값 가져오기
    ...
    switch (key) {
      ...
      case 'ArrowUp':
        ...
        break
      case 'ArrowDown':
        if (rowIndex <= GRID.ROWS) rowIndex += 1
        break
      case 'Escape':
        console.log('Esc')
        break
