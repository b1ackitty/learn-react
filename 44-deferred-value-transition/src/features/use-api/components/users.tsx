...
// 용도1. useContext 훅을 대신할 수 있음(Context로부터 값을 가져올 수 있음)
// 용도2. Promise 인스턴스를 전달받아 Promise의 보류/해결/거절 등 상태에 따른 처리 가능
//        보류 중일 때는 Suspense의 fallback이 대체 UI로 화면에 렌더링(로딩)
//        해결될 때까지 use 함수는 컴포넌트 렌더링을 차단(진행하지 않음) - 동기 코드인 것처럼
//        해결되면 use 함수는 Promise의 데이터를 반환함(컴포넌트 내부에서 사용) - 컴포넌트 렌더링
...
export default function Users() {
  ...
  // [안전] Promise 인스턴스를 기억하고 있으므로 렌더링과 상관 없이 동일한 참조를 하위 컴포넌트에 전달
