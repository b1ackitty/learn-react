  ...
  if (error) {
    return (
      <div
        role="alert"
        aria-live="assertive"
        className="fles flex-col gap-2 text-2xl bg-red-600 text-red-50 p-4 fixed top-0 left-0 right-0"
      >
        <h3 className="text-xl font-semibold">{error.name}</h3>
        <p className="text-base">{error.message} 추후 다시 시도해 주세요.</p>
      </div>
    )
  }
