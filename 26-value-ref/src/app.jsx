...
function MemoPrevStateValue() {
  ...
  return (
    <div className="p-10 bg-white text-blue-950 text-2xl">
      <p>현재 상태 값: {count}</p>
      <p>
        이전 상태 값: {prevCountRef.current ?? 'undefined'} (참조 객체의 current
        값)
        {/* 이전 상태 값: {prevCount ?? 'undefined'} */}
      </p>
      <button
        type="button"
        className="button"
        onClick={() => {
          ...
          // setPrevCount(count) // 다음 렌더링 시점엔 이전 상태 값
          prevCountRef.current = count
        }}
      >
        count 증가
...
function FunctionalComponent() {
  // 지역 변수는 렌더링될 때마다 값이 초기화 값 기억 ❌
  // let message = '헬로 리액트!'

  // useRef 훅을 사용해 값 참조(기억) ✅
  const messageRef = useRef('HELLO 리액트!') // 일반 JS Object { current: 초기값 }
